...
- ownership is repaired with the Hermes image's own `hermes` UID/GID during updates and before service start, so image UID changes do not leave the volume unwritable; package updates additionally run `hermes update` as the `hermes` user after the ownership repair
