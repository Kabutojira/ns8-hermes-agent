...
Restart supervision is owned by `hermes@<id>.service`, `openwebui@<id>.service`, and `hermes-auth.service` with `Restart=on-failure`; the Podman pod and container launches do not set container-level restart policies.
...
If `base_virtualhost` is set, Traefik forwards `https://<base_virtualhost>/` to the shared auth listener on `TCP_PORT` using the route instance `<module>-hermes-auth`. The auth proxy maps `/` to the authenticated session's Hermes dashboard and `/chat` to the same session's Open WebUI sidecar; `/hermes-<id>/dashboard` and `/hermes-<id>/chat` remain explicit per-agent paths.
